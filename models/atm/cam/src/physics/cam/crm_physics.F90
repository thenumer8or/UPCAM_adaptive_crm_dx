...
! modal_aero_data only exists if MODAL_AERO
   ...
   use crmdims,         only: crm_dx, crm_dy, crm_dt
...
! CRM column radiation stuff:
   ...
   real(r8) crm_dx_curr(pcols) ! Adaptive crm_dx
#endif
...
! Initialize stuff:
   ...
   if(is_first_step()) then
#if defined(ADAPTIVECRMDX) .OR. defined(BULLSEYECRMDX)
   crm_dx_curr = crm_dx ! start off with 
#endif 
...
!      print*,'precc:',minval(precc(:ncol))*86400000.,maxval(precc(:ncol))*86400000.
       ...
       call outfld ('DX_CRM',crm_dx_curr(:), pcols, lchnk)
