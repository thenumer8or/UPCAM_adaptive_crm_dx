...
#if defined(ADAPTIVECRMDX) .OR. defined(BULLSEYECRMDX)
                       crm_dx_curr, &
#endif
                       qrad_crm, &
...
#if defined(ADAPTIVECRMDX) .OR. defined(BULLSEYECRMDX)
         real(r8), intent(inout) :: crm_dx_curr
#endif
...
! date: 8/29/2015.
        ...
        integer kx
        real(r8)             :: umax = 0.5*crm_dx/crm_dt ! maxumum ampitude of the l.s. wind   !HP modifies this line to port Marat's adaptive dx scheme.
        !real(r8), parameter :: umax = 0.5*crm_dx/crm_dt ! maxumum ampitude of the l.s. wind   !original line of the code.
...
! HP is wondering maybe this dx initialization should be happening before the main loop, not inside of it.
#if defined(ADAPTIVECRMDX) .OR. defined(BULLSEYECRMDX)
  dx = crm_dx_curr
#endif

#ifdef BULLSEYECRMDX
  call bullseye_dx_change (latitude0,longitude0)   ! statically change horizonal grid size, call only once before the loop.
#endif
...
#ifdef ADAPTIVECRMDX
!  dx = crm_dx_curr
!  write (0,*) "Inside crm_module flag1. dx = ", dx

  call dx_change()   ! dynamically change horizonal grid size
#endif
...
#if defined(ADAPTIVECRMDX) .OR. defined(BULLSEYECRMDX)
       crm_dx_curr = dx
!       write (0,*) "Inside crm_module flag 2. dx = ", dx
#endif
